...
Возвращаясь к символам, нам остаётся создать такой класс **Приспособленец**, для каждого символа, скажем ASCII. При таком подходе число объект будет куда меньше, чем если бы мы создавали объекты под каждый символ в тексте. Теперь каждый символ текста будет ссылать на конкретный, нужный ему символ(объект **Приспособленец**) и передавать ему внешнее состояние.  
